<svg class=" pointer-events-none z-[1000000000000000]" id="motionSimulation" viewBox="0 0 200 100"
...
function spark(e, opt_properties) {
    ...
    for (let i = 0; i < c.sparks; i++) {
        svg.insertAdjacentHTML('afterbegin',
            `<path d="M50 50 50 ${50 - c.sw / 2}" stroke="${col}" stroke-linecap="round" stroke-width="${c.sw}" fill="none" transform="rotate(${((360 / c.sparks) * i) - (180 / c.sparks)} 50 50)"><animate attributeName="d" values="M50 50 50 ${50 - c.sw / 2}; M50 ${50 - c.sw} 50 ${c.sw / 2}; M50 ${c.sw / 2} 50 ${c.sw / 2}" dur="${c.time}ms" begin="0s" repeatCount="0" fill="freeze" /></path>`
        );
    }
...
infiniteSparkle(); // Start the automatic sparkles
